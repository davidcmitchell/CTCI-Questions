...
pair<Node<int>*, Node<int>*> isAncestor(Node<int> *p, Node<int> *q, Node<int> *curr)
{
    pair<Node<int>*,Node<int>*> ret_val = pair<Node<int>*,Node<int>*>(nullptr,nullptr);
    if (curr == nullptr)
        return ret_val;
    pair<Node<int>*,Node<int>*> lc = isAncestor(p,q,curr->left_child);
    pair<Node<int>*,Node<int>*> rc = isAncestor(p,q,curr->right_child);
    if (lc.first != nullptr && lc.second != nullptr)
    {
        return lc;
    }
    if (rc.first != nullptr && rc.second != nullptr)
    {
        return rc;
    }
    if (lc.first != nullptr || rc.first != nullptr || curr == p)
    {
        ret_val.first = curr;
    }
    if (lc.second != nullptr || rc.second != nullptr || curr == q)
    {
        ret_val.second = curr;
    }
    return ret_val;
...
int main(int argc, char* argv[])
{
    ...
    root->right_child->right_child = new Node<int>(4);

    printTree(root,4);
    pair<Node<int>*,Node<int>*> anc = isAncestor(root->right_child->right_child,root->right_child,root);
    return 0;
}